...
const getCurriculum = (): Curriculum => {
  const cv = fs.readFileSync(`${process.cwd()}/src/data/cv.yml`)
...
const Main = ({ data }: { data: Curriculum }) => {
  ...
  return (
    <div className="h-[277mm] flex flex-col">
      ...
      <h3 className={`text-md text-center uppercase ${titleFont.className}`}>
        {data.city} &middot; {yearsOfExperience} années d&rsquo;expérience
      </h3>

      <Title className="mb-5 mt-3 uppercase">
        ...
        {data.projects.map((project, index) => (
          <div key={project.dates} className="relative">
            <div className="mb-2">
              ...
              {project.stack ? (
                <Markdown className="italic m-0">{project.stack}</Markdown>
              ) : null}
            </div>
            <div className="flex">
              <Markdown className="[&_ul]:m-0 [&_ul]:pl-5">
                {project.description}
              </Markdown>
            </div>
          </div>
