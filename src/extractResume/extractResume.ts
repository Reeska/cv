...
const mapBaseCard = async (experience: ElementHandle<SVGElement | HTMLElement>): Promise<Card> => {
  ...
  const dates = (await (await experience.$('.date-range'))?.textContent())?.trim() ?? '';
  const duration = (await (await experience.$('.before\\:middot'))?.textContent())?.trim() ?? '';
  ...
  const description = await (descriptionMore ? descriptionMore : descriptionLess)?.innerHTML() ?? '';
  return {
    title,
    dates: dates.replace(duration, '').replace(/\n/g, '').trim(),
    duration,
    description: cleanHTML(description.slice(0, description.indexOf('<button class')).trim()),
  };
};

const mapCard = async (experience: ElementHandle<SVGElement | HTMLElement>): Promise<CardWithCompany> => {
  const company = (await (await experience.$('h4'))?.textContent())?.trim() ?? '';
  const companyPictureUrl = await (await experience.$('.profile-section-card__image'))?.getAttribute('src') ?? undefined;
  return {
...
const mapGroup = async (group: ElementHandle<SVGElement | HTMLElement>): Promise<CardWithCompany[]> => {
  const company = (await (await group.$('.experience-group-header__company'))?.textContent())?.trim() ?? '';
  const companyPictureUrl = getOverridePicture(company) ?? await (await group.$('img'))?.getAttribute('src') ?? undefined;
...
const mapExperience = async (experience: ElementHandle<SVGElement | HTMLElement>): Promise<CardWithCompany | CardWithCompany[]> => {
...
const elementToBase64Picture = async (promisedElement: Promise<ElementHandle<SVGElement | HTMLElement> | null>) => {
...
test('extract resume and generate json, html and pdf', async ({page}) => {
  ...
  const experiencesTitles = await page.$$('.experience__list h3, .education__list h3');
  ...
  const fullUrl = (await (await page.$('[rel=canonical]'))?.getAttribute('href')) ?? '';
  const url = fullUrl.slice(fullUrl.indexOf('linkedin'));
  const pictureUrl = (await (await page.$('.top-card__profile-image'))?.getAttribute('src')) ?? undefined;

  const rawExperiences = await page.$$('.experience-item, .experience-group');
  const projects = (await Promise.all(rawExperiences.map(mapExperience))).flat() as CardWithCompany[];

  const rawFormations = await page.$$('.education__list-item');
  const formations = await Promise.all(rawFormations.map(mapFormation));

  const template = fs.readFileSync('src/extractResume/template.html').toString('utf-8');
  ...
  const projectsWithPictures = await Promise.all(data.projects.map(async (project) => {
    return {
      ...project,
      companyPicture: await elementToBase64Picture(page.$(`[id="${project.dates}"]`))
    };
  }));

  const formationsWithPictures = await Promise.all(data.formations.map(async (formation) => {
    return {
      ...formation,
      schoolPicture: await elementToBase64Picture(page.$(`[id="${formation.dates}"]`))
    };
  }));
  ...
  // fs.writeFileSync('out/cv.html', cv);
  // fs.writeFileSync('src/data/cv.json', JSON.stringify(dataWithPictures, undefined, 2));
  fs.writeFileSync('src/data/cv.yml', '# yaml-language-server: $schema=./cv.yml \n' + YAML.stringify(dataWithPictures, undefined));

  // await page.pdf({path: 'out/cv.pdf', printBackground: true});
});
